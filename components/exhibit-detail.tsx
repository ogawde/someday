...
export function ExhibitDetail({
  ...
  exhibit: ExhibitWithOwner;
  session: AppSession;
  hasSentMessage: boolean;
}) {
  const username = exhibit.owner.username ?? "anonymous";
  const isOwner = session?.user.id === exhibit.owner.id;
  const images = exhibit.media.filter((media) => media.type === "image");
  const links = exhibit.media.filter((media) => media.type === "link");
